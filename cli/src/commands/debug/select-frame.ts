...
export function selectFrameCommand(program: Command) {
  program
    .command("select-frame")
    .description("Select stack frame")
    .requiredOption("--index <index>", "Frame index")
    ...
    .action(async (options, cmd) => {
      ...
      const params: Record<string, unknown> = {
        frameIndex: parseInt(options.index),
      };
      ...
      const response = await client.callTool("ide_select_stack_frame", params);
