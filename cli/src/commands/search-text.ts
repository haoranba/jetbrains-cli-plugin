...
export function searchTextCommand(program: Command) {
  program
    .command("search-text")
    .description("Text search using IDE's word index")
    .requiredOption("--query <query>", "Text to search for")
    ...
    .action(async (options, cmd) => {
      ...
      const response = await client.callTool("ide_search_text", args);
